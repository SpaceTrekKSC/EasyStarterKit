...
#define TOUCH_OUT1 10//out1-TCH1 area corresponds to play song operation
#define TOUCH_OUT2 13//out2-TCH2 area corresponds to pause operation
#define TOUCH_OUT3 11//out3-TCH3 area corresponds to next song operation
#define TOUCH_OUT4 12//out4-TCH4 area corresponds to previous song operation
  ...
  if(button != -1)// there is a touch
  {
	...
	if(button == touch.get())// Check again if there is a touch
	{
	  ...
	  switch(button)
	  {
		...
		case TOUCH_OUT3: mp3.nextSong();break;//touch TCH3 area
		case TOUCH_OUT4: mp3.previousSong();break;//touch TCH4 area
		default: break;
	  }
	}
