...
RGB::RGB(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), pixels(NULL)
...
uint8_t g, uint8_t r, uint8_t b, uint16_t n) {
  if(n < numLEDs) {
...
void RGB::setPixelColor(uint32_t c, uint16_t n) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t RGB::Color(uint8_t g, uint8_t r, uint8_t b) {
