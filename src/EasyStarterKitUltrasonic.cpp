...
Ultrasonic::Ultrasonic(uint8_t tp, uint8_t ep)
    {
    pinMode(tp, OUTPUT);
    pinMode(ep, INPUT);
    _trigPin = tp;
...
float Ultrasonic::readDistance()
{
  ...
  float cmMsec;
  cmMsec = convert(microsec,CM);
  if(cmMsec > 500) return -1;
  cmMsec = ( (float)( (int)( (cmMsec + 0.05) * 10 ) ) ) / 10;//Rounded to one decimal place
  return cmMsec;
}
